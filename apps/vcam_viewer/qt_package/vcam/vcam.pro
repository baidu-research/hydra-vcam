...
win32: {
    ...
    CONFIG(debug, debug|release) {
        ...
        LIBS += -L$$PWD/ncc_sdk/ -lvcamd
        PRE_TARGETDEPS += $$PWD/ncc_sdk/vcamd.lib
    } else {
        LIBS += C:\opencv34\opencv\build\x64\vc14\lib\opencv_world340.lib
        LIBS += -L$$PWD/ncc_sdk/ -lvcam
        PRE_TARGETDEPS += $$PWD/ncc_sdk/vcam.lib
    }
...
unix:{
    ...
    PKGCONFIG += libusb-1.0
      ...
      contains(QMAKE_HOST.arch, x86_64){
        ...
        LIBS += \
        -L$$PWD/ncc_sdk/ -lvcam -lavutil -lavformat -lavcodec -lswresample -lswscale

      }else{
        message("build on arm64")
        INCLUDEPATH += /usr/include/arm-linux-gnueabihf
     LIBS += \
        -L$$PWD/ncc_sdk/ -lvcam_rasp
