    ...
    , ui(new Ui::Widget)
{
    ...
    connect(ui->web_label3,&browserlabel::OpenBrowser,[=](){
        QDesktopServices::openUrl(QUrl(QString("https://www.vcam.com.cn/")));
    });
...
void Widget::on_load_fw_btn_clicked()
{
    ...
    if(!ui->mjpeg_stream->isChecked()&&!ui->yuv_stream->isChecked()&&!ui->h264_stream->isChecked()&&!ui->h265_stream->isChecked()){
        ...
        ret = loadFw("/Configuration/moviUsbBoot","/Configuration/fw/vcam.mvcmd");


        if (ret<0)
        {
            QApplication::restoreOverrideCursor();
            ui->log_area->append("load firmware to vcam failed!");
        }
        else {
